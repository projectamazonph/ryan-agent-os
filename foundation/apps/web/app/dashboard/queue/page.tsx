...
import { getImplementationQueue, getToken } from "@/lib/api";

export default async function QueuePage() {
  const token = await getToken();
  if (!token) redirect("/login");
  const tasks = await getImplementationQueue();
    ...
    <div className="shell">
      ...
      <main className="main">
        ...
        <section className="card">
          ...
          <div className="card-body"><ImplementationQueue tasks={tasks} /></div>
        </section>
      </main>
    </div>
  );
