...
Current private-MVP enforcement includes workspace ownership, ready-task checks, immutable agent policies, per-run tool allowlists, guarded run transitions, and QA-only success promotion. This is not yet a complete approval policy engine.
...
- Agent runs receive bounded context instead of full workspace history.
- Definition and context identities cannot be silently changed after launch.
- Both denied and allowed tool requests are retained.
- The current tool endpoint authorizes and records requests but does not execute connectors.
- Live traces require the authenticated session and expose only workspace-owned runs.
- Protected external actions remain unavailable until approval requests are enforced.
